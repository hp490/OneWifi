...
static bus_error_t cacr_set_client_acl_rules(char *event_name, raw_data_t *p_data, void *userData)
{
    ...
    ret = parse_client_assoc_ctrl_json(json_buf, &bssid, &assoc_control, &validity_period, sta_mac_list, &sta_count);
    free(json_buf);
	...
	kick_details->cancelled_by_unblock = false;
        ...
        if (add_pending_block(kick_details) != RETURN_OK) {
            /* We couldn't track this block, so roll back the ACL entry and expedite timer cleanup
             * to restore the original filter mode and free kick_details safely via del_acl_cb().
             */
